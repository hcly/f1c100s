 ...
 */
static void spi_nand_ecc_status_gigadevice(unsigned int status,
                                        unsigned int *corrected,
                                        unsigned int *ecc_error)
{
	unsigned int ecc_status = status & SPI_NAND_GD5F_ECC_MASK;

	*ecc_error = (ecc_status == SPI_NAND_GD5F_ECC_UNCORR);
	switch (ecc_status) {
	case SPI_NAND_GD5F_ECC_0_BIT:
		*corrected = 0;
		break;
	case SPI_NAND_GD5F_ECC_1_7_BIT:
		*corrected = 7;
		break;
	case SPI_NAND_GD5F_ECC_8_BIT:
		*corrected = 8;
		break;
	}
}
				...
				u32 column, bool ecc_off, unsigned int *corrected, u_char *buf, size_t len)
{
	...
	if (!ecc_off) {
		if( chip->options & SPINAND_USED_GIGADEVICE )
			spi_nand_ecc_status_gigadevice(status, corrected, &ecc_error);
		else
...
static void spi_nand_set_rd_wr_op(struct spi_nand_chip *chip)
{
	...
	else if( chip->options & SPINAND_USED_GIGADEVICE )
		chip->read_cache_op = SPINAND_CMD_READ_FROM_CACHE;
	else
...
static int spi_nand_init(struct spi_slave *spi, struct spi_nand_chip **chip_ptr)
{
	...
	struct spi_nand_chip *chip = *chip_ptr;
	if( chip == NULL )
	{
...
int spi_nand_cmd_erase_ops(struct spi_nand_chip *chip, u32 offset, size_t len, bool spread)
{
	...
	while (leftlen > 0) {
		...
		if (spread)
			...
			/* try to erase one bad block */
			if(end)
				spi_nand_erase(chip, eraseoffset, chip->block_size);
		}
	}
